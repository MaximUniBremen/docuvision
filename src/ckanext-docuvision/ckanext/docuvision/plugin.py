...
class DocuvisionPlugin(plugins.SingletonPlugin):
    ...
    def get_actions(self):
        return {
            'docuvision_process_document': lambda context, data_dict: self.docuvision_process_document(context, data_dict)
        }
    ...
    def _process_resource(self, resource):
        ...
        if ext_fmt != fmt and ext_fmt in format_map.values():
            fmt = ext_fmt  # Prefer the actual file extension

        log.info(f"Resource format field: {resource.get('format')}, file/url: {url}, resolved fmt: {fmt}")

        upload = ResourceUpload(resource)
        filepath = upload.get_path(resource['id'])

        if not os.path.isfile(filepath):
            log.error(f"File not found for resource {resource['id']}: {filepath} (resource: {resource})")
        ...
        if fmt == 'pdf':
            ...
            log.error(f"Unsupported format for text extraction: format field='{resource.get('format')}', resolved fmt='{fmt}', resource={resource}")
    ...
    def _extract_text_doc(self, filepath):
        ...
        try:
            ...
            try:
                ...
                log.warning(f"Textract failed for DOC: {textract_err}, trying antiword fallback.")
                ...
                try:
                    ...
                    log.error(f"Both textract and antiword failed for DOC: {antiword_err}")
                    raise Exception(f"Failed to extract DOC text: textract error: {textract_err}, antiword error: {antiword_err}")
        except Exception as e:
    ...
    def _extract_text_xlsx(self, filepath):
        ...
        try:
            ...
            log.warning(f"openpyxl failed on {filepath} with error: {e}, trying xlrd as fallback.")
        ...
        try:
            ...
            log.error(f"Both openpyxl and xlrd failed to read Excel file {filepath}: {e2}")
            raise Exception(f"Failed to extract text from Excel file. openpyxl error: {e}, xlrd error: {e2}")
